...
Original source: agent/memory_provider.py
...
class MemoryProvider(ABC):
    ...
    def memory_layer(self) -> str:
        """Voice runtime layer: core, session, or external.

        Hermes treats "builtin" as the local provider and allows one external
        provider. The realtime voice runtime keeps the same one-external rule,
        but splits local memory into core and session providers so the prompt
        contract can mirror Hermes concepts explicitly.
        """
        return "external"
    ...
    async def ahandle_tool_call(self, tool_name: str, args: Dict[str, Any], **kwargs) -> str:
        ...
        Default delegates to the sync version. Override for providers that
        need to call async backends (e.g. MemoryService).
        """
